...
#
# is urn:miriam:biomodels.db:BIOMD0000000223
# is urn:miriam:biomodels.db:MODEL6194251662
# isDescribedBy urn:miriam:pubmed:19357636
#
...
protein_idx = [21,22,15,30,40,41,43,44,39,46,47,36,49];
protein_names = {'IR-21','IR-22','RasGAP-15','SHP2-30','RAS-40','RAF-41','MEK2-43','ERK-44','PIP3','PDK1','AKT','mTOR'};

times_to_plot = [1,11,101,301,1000];
to_plot = [1,3,4,6,7,8,11];
protein_idxs_to_plot = zeros(1,length(to_plot));
legends = cell(1,length(to_plot));
k = 1;
for i = to_plot
	legends{k} = protein_names{i};
	protein_idxs_to_plot(k) = protein_idx(i);
    k = k +1;
endfor	

res = x(times_to_plot,protein_idxs_to_plot);
x0 = x(times_to_plot(1),protein_idxs_to_plot);
